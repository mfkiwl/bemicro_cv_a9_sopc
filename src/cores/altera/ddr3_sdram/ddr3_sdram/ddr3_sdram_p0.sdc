...
#      in the ddr3_sdram_p0_timing.tcl script.
#    * The helper routines are defined in ddr3_sdram_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/ddr3_sdram_p0_timing.tcl"
...
set half_period [ ddr3_sdram_p0_round_3dp [ expr $t(CK) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ ddr3_sdram_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]
...
set data_output_min_delay [ ddr3_sdram_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ ddr3_sdram_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]
...
set data_input_max_delay [ ddr3_sdram_p0_round_3dp [ expr $t(rdu_input_max_delay_external) + $t(rdu_input_max_delay_internal) ]]
...
set data_input_min_delay [ ddr3_sdram_p0_round_3dp [ expr - $t(rdu_input_min_delay_external) - $t(rdu_input_min_delay_internal) ]]

# Minimum delay on address and command paths
set ac_min_delay [ ddr3_sdram_p0_round_3dp [ expr - $t(IH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) - $ISI(addresscmd_hold) ]]

# Maximum delay on address and command paths
set ac_max_delay [ ddr3_sdram_p0_round_3dp [ expr $t(IS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) + $ISI(addresscmd_setup) ]]
...
if { ! [ info exists ddr3_sdram_p0_sdc_cache ] } {
	set ddr3_sdram_p0_sdc_cache 1
	ddr3_sdram_p0_initialize_ddr_db ddr3_sdram_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names ddr3_sdram_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $ddr3_sdram_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_ddr3_sdram_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	set final_data_input_max_delay [ ddr3_sdram_p0_round_3dp [ expr $data_input_max_delay + $fpga(tDQS_PSERR) ]]
	set final_data_input_min_delay [ ddr3_sdram_p0_round_3dp [ expr $data_input_min_delay - $t(CK) / 2.0 + $t(QH_time) - $fpga(tDQS_PSERR) - $tJITper]]

	if { $debug } {
		post_message -type info "SDC: Jitter Parameters"
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_ddr3_sdram_p0_dqs_delay_chain_length"
	...
	# AFI clock
	set local_pll_afi_clk [ ddr3_sdram_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_AFI_CLK) ]
	


	# DQ write clock
	set local_pll_dq_write_clk [ ddr3_sdram_p0_get_or_add_clock_vseries \
		-target $pll_dq_write_clock \
		-suffix "dq_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_WRITE_CLK) ]
	
	# DQS write clock
	set local_pll_write_clk [ ddr3_sdram_p0_get_or_add_clock_vseries \
		-target $pll_write_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_MEM_CLK) \
		-divide_by $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_MEM_CLK) \
		-phase $::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_MEM_CLK) ]


	# NIOS clock
	set local_pll_avl_clock [ ddr3_sdram_p0_get_or_add_clock_vseries \
		-target $pll_avl_clock \
		-suffix "avl_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_NIOS_CLK) \
		-divide_by $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_NIOS_CLK) \
		-phase $::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_NIOS_CLK) ]

	set mem_factor [expr double($::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_MEM_CLK)) / $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_MEM_CLK)]
	set avl_factor [expr double($::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_NIOS_CLK)) / $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_NIOS_CLK)]
	...
	if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
		set local_pll_avl_phy_clk [ ddr3_sdram_p0_get_or_add_clock_vseries \
	...
	set local_pll_config_clock [ ddr3_sdram_p0_get_or_add_clock_vseries \
		-target $pll_config_clock \
		-suffix "config_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_ddr3_sdram_p0_pll_mult(PLL_CONFIG_CLK) \
		-divide_by $::GLOBAL_ddr3_sdram_p0_pll_div(PLL_CONFIG_CLK) \
		-phase $::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_CONFIG_CLK) ]	



	# Pulse-generator used by DQS tracking
	set local_sampling_clock "${inst}|ddr3_sdram_p0_sampling_clock"
	...
	if {[string compare -nocase $pll_driver_core_clock "_UNDEFINED_PIN_"] != 0} {
		set local_pll_driver_core_clk [ ddr3_sdram_p0_get_or_add_clock_vseries \
	...
	foreach { ck_pin } $ck_pins { 
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -max [ddr3_sdram_p0_round_3dp [expr $t(CK) - $t(DQSS)*$t(CK) - $board(minCK_DQS_skew) ]] $dqs_pins
		set_output_delay -add_delay -clock [get_clocks $ck_pin] -min [ddr3_sdram_p0_round_3dp [expr $t(DQSS)*$t(CK) - $board(maxCK_DQS_skew) ]] $dqs_pins
		...
		foreach { ac_pin } $ac_pins {
			...
			if {[get_collection_size $ac_port] > 0} {
				if [ get_port_info -is_output_port $ac_port ] {
					...
					set_output_delay -min [ddr3_sdram_p0_round_3dp [expr {$ac_min_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] $ac_port -add_delay

					# Specifies the maximum delay difference between the DQS pin and the address/control pins:
					set_output_delay -max [ddr3_sdram_p0_round_3dp [expr {$ac_max_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] $ac_port -add_delay
				}
			}
		}
	}
	...
	if {$::GLOBAL_ddr3_sdram_p0_pll_phase(PLL_NIOS_CLK) > 0} {
		if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
...
if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
	set dqs_clocks [ddr3_sdram_p0_get_all_instances_dqs_pins ddr3_sdram_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_ddr3_sdram_p0_corename}_report_timing.tcl]]"
