...
source "$script_dir/ddr3_sdram_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	# DQS_phase offset
	set dqs_phase [ ddr3_sdram_p0_get_dqs_phase $dqs_pins ]

	set fitter_run [ddr3_sdram_p0_get_io_interface_type [lindex [lindex $pins(q_groups) 0] 0]]
	if {$fitter_run == ""} {
	...
	set interface_type [ddr3_sdram_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [ddr3_sdram_p0_get_io_standard [lindex $dqs_pins 0]]
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_ddr3_sdram_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_ddr3_sdram_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [ddr3_sdram_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_ddr3_sdram_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_ddr3_sdram_p0_corename}-$inst)
			...
			set assumptions_valid [ddr3_sdram_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			set assumptions_cache(${::GLOBAL_ddr3_sdram_p0_corename}-$inst) $assumptions_valid
		}	
		...
		ddr3_sdram_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family scale_factors $interface_type $max_package_skew $dll_length $period pins t summary MP IP board

		#######################################
		# Read Analysis

		ddr3_sdram_p0_perform_flexible_read_capture_timing_analysis $opcs $opcname $inst $family scale_factors $io_std $interface_type $max_package_skew $dqs_phase $period $all_dq_pins pins t summary MP IP board fpga

		#######################################
		# PHY and Address/command Analyses

		ddr3_sdram_p0_perform_ac_analyses  $opcs $opcname $inst scale_factors pins t summary IP		
		ddr3_sdram_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP


		#######################################
		# Bus Turnaround Time Analysis
		ddr3_sdram_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI


		#######################################
		# Postamble analysis
		ddr3_sdram_p0_perform_flexible_postamble_timing_analysis $opcs $opcname $inst scale_factors $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $DQSpathjitter pins t summary MP IP SSN board ISI
	...
	set summary [lsort -command ddr3_sdram_p0_sort_proc $summary]

	set f -1
	if { [ddr3_sdram_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_ddr3_sdram_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_ddr3_sdram_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ ddr3_sdram_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ ddr3_sdram_p0_round_3dp $hold]
		}
